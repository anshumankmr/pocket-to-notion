...
class TokenBucket:
    ...
    def __init__(self, max_bucket_size, current_bucket_size,
                 refill_rate=1, last_refill_time_stamp=time.time()):
    ...
    def allow_request(self, tokens):
        print("Before", self.current_bucket_size, tokens,
              self.max_bucket_size, self.refill_rate,
              self.last_refill_time_stamp)
        print(self.refill())
        if int(self.current_bucket_size) >= int(tokens):
            self.current_bucket_size = int(
                self.current_bucket_size) - int(tokens)
            print("After", self.current_bucket_size, tokens,
                  self.max_bucket_size, self.refill_rate,
                  self.last_refill_time_stamp)
            return True
    ...
    def refill(self):
        ...
        print(time_passed)
        tokens_to_add = int((time_passed * int(self.refill_rate))//(10**4))
        print(tokens_to_add)
        self.last_refill_time_stamp = now
        self.current_bucket_size = min(
            int(self.max_bucket_size),
            int(tokens_to_add) + int(self.current_bucket_size))
        return {}
